...
(function(window, undefined) {
	...
	jQuery.support = (function() {
		...
		support = {
			...
			// Make sure unknown elements (like HTML5 elems) are handled
			// appropriately
		...
		jQuery(function() {
			var container, outer, inner, table, td, offsetSupport, conMarginTop = 1, ptlm = "position:absolute;top:0;left:0;width:1px;height:1px;margin:0;", vb = "visibility:hidden;border:0;", style = "style='"
					+ ptlm + "border:5px solid #000;padding:0;'", html = "<div "
					+ style
					+ "><div></div></div>"
					+ "<table "
					+ style
			...
			.extend({
				...
				data : function(elem, name, data, pvt /* Internal Use Only */) {
					...
					id = isNode ? elem[jQuery.expando] : elem[jQuery.expando]
							&& jQuery.expando, isEvents = name === "events";
	...
	var rnamespaces = /\.(.*)$/, rformElems = /^(?:textarea|input|select)$/i, rperiod = /\./g, rspaces = / /g, rescape = /[^\w\s.|`]/g, rtypenamespace = /^([^\.]*)?(?:\.(.+))?$/, rhoverHack = /\bhover(\.\S+)?/, rkeyEvent = /^key/, rmouseEvent = /^(?:mouse|contextmenu)|click/, rquickIs = /^(\w*)(?:#([\w\-]+))?(?:\.([\w\-]+))?$/, quickParse = function(
			selector) {
	...
	jQuery.event = {
		...
		add : function(elem, types, handler, data, selector) {
			...
			for (t = 0; t < types.length; t++) {
				...
				// Delegated event; pre-analyze selector so it's processed
				// quickly on event dispatch
				if (selector) {
		...
		remove : function(elem, types, handler, selector) {
			...
			for (t = 0; t < types.length; t++) {
				...
				if (!type) {
					...
					for (j in events) {
						jQuery.event.remove(elem, j + namespaces, handler,
								selector);
					}
					return;
				}
				...
				if (handler || namespaces || selector || special.remove) {
					for (j = 0; j < eventType.length; j++) {
						...
						if (!handler || handler.guid === handleObj.guid) {
							if (!namespaces
									|| namespaces.test(handleObj.namespace)) {
								if (!selector
		...
		dispatch : function(event) {
			...
			var handlers = ((jQuery._data(this, "events") || {})[event.type] || []), delegateCount = handlers.delegateCount, args = [].slice
					...
					&& !event.namespace, specialHandle = (jQuery.event.special[event.type] || {}).handle, handlerQueue = [], i, j, cur, ret, selMatch, matched, matches, handleObj, sel, hit, related;
				...
				for (cur = event.target; cur != this; cur = cur.parentNode
					...
					for (i = 0; i < delegateCount; i++) {
						...
						if (handleObj.isPositional) {
							...
							// http://jsfiddle.net/eJ4yd/3/
							hit = (hit || (selMatch[sel] = jQuery(sel)))
									.index(cur) >= 0;
						} else if (hit === undefined) {
							hit = selMatch[sel] = (handleObj.quick ? quickIs(
									cur, handleObj.quick) : jQuery(cur).is(sel));
						}
						if (hit) {
		...
		mouseHooks : {
			props : "button buttons clientX clientY fromElement offsetX offsetY pageX pageY screenX screenY toElement wheelDelta"
					.split(" "),
			filter : function(event, original) {
					...
					function(orig, fix) {
						jQuery.event.special[orig] = jQuery.event.special[fix] = {
							...
							handle : function(event) {
								...
								// For a real mouseover/out, always call the
								// handler; for
								// mousenter/leave call the handler if related
								// is outside the target.
								// NB: No relatedTarget if the mouse
								// left/entered the browser window
								if (!related
										|| handleObj.origType === event.type
	...
	jQuery.fn
			.extend({
				...
				stop : function(type, clearQueue, gotoEnd) {
					...
					return this
							.each(function() {
								var i, hadTimers = false, timers = jQuery.timers, data = jQuery
										._data(this);
